    ...
    while True:
        ...
        if in_packet and match(dom, HANDSHAKE_END_HZ):
            ...
            try:
                ...
                if bytearray(b'201402367') in byte_stream: 
                        #print("201402367 in byte_stream") 
                        byte_stream = re.sub(b'201402367',b'',byte_stream) 
                        #print("replace byte_stream : ",byte_stream)
                        byte_stream = RSCodec(FEC_BYTES).encode(byte_stream) 
                        newbyte_stream = [] 
                        newbyte_stream.append(HANDSHAKE_START_HZ) 
                        newbyte_stream.append(HANDSHAKE_START_HZ) 
                        for i in range(len(byte_stream)):
                            newbyte_stream.append(((byte_stream[i] >> 4)*STEP_HZ)+START_HZ) 
                            newbyte_stream.append(((byte_stream[i] & 0xf)*STEP_HZ)+START_HZ)
                        newbyte_stream.append(HANDSHAKE_END_HZ) 
                        newbyte_stream.append(HANDSHAKE_END_HZ) 
                        ...
                        for i in range(len(newbyte_stream)):
                            samples = (np.sin(2*np.pi*np.arange(44100*duration)*(newbyte_stream[i]/44100))).astype(np.float32)
                            stream.write(samples)
                        stream.stop_stream() 
                        stream.close() 
                        p.terminate()  
                else: 
                        print("학번이 포함되어있지 않습니다")
